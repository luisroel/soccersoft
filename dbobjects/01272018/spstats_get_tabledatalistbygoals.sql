drop procedure if exists `spstats_get_tabledatalistbygoals`;
delimiter $$
create procedure`spstats_get_tabledatalistbygoals`(
	  in from_p datetime
    , in to_p	datetime
)
begin
	...
	from (
		...
		from
			...
			inner join `tbstats_goalsprojection` `gp` on
				`gp`.`fixtureid` = `td`.`fixtureid`
