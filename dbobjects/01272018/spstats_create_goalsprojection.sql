drop procedure if exists `spstats_create_goalsprojection`;
delimiter $$
create procedure `spstats_create_goalsprojection`(
	  in from_p datetime
    , in to_p	datetime
)
BEGIN
	delete from `tbstats_goalsprojection` where `fixtureid` in (select `ft`.`fixtureid` from `tbcore_fixture` `ft` where `ft`.`time` >= from_p and `ft`.`time` <= to_p);
    set @from_date = from_p;
    while (@from_date < to_p) do
		set @to_date = adddate(@from_date, interval 86399 second);
		insert into	tbstats_goalsprojection
		select
			  `tt`.`fixtureid`
/*			, `aa`.`shortname`	as `ct`
			, `tn`.`shortname`	as `lg`
