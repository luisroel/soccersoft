...
BEGIN
	...
	IF EXISTS(SELECT * FROM tbCore_Tournament CC WHERE CC.ShortName = ShortName_p AND CC.TournamentId <> TournamentId_p) THEN
		SET Msg_p = 'Tournament already exist. Please verify!';
	ELSE IF NOT EXISTS(SELECT * FROM tbCore_Tournament CC WHERE CC.TournamentId = TournamentId_p) THEN
