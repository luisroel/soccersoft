...
--  Home Wins Matches
--
DROP VIEW IF EXISTS vwStats_Get_Home7Raw;
CREATE VIEW vwStats_Get_Home7Raw AS
SELECT
	  SS.SeasonId
    ...
    , fnStats_Get_Odds('L', FT.Odd_1, FT.Odd_X, FT.Odd_2)			AS 'Odd'
...
WHERE
--		(PA.OH - PA.OV) BETWEEN -0.57 AND 0.73				-- Original
		(PA.OH - PA.OV) BETWEEN -0.496 AND 1.159
	AND IFNULL(PF.OH,0) - IFNULL(PF.OV,0) > 0
	AND FT.Last5_V LIKE 'P%'
    AND FT.WeekNo > 4
