...
--  Home Wins Matches
--
DROP VIEW IF EXISTS vwStats_Get_All7Raw;
CREATE VIEW vwStats_Get_All7Raw AS
SELECT
	  SS.SeasonId
    ...
    , fnStats_Get_FCHandicap(PA.Pb_Home, PA.Pb_Draw, PA.Pb_Visit, 0.11) 	AS `FC`
    , fnStats_Get_Result(FT.ScoredHome, FT.ScoredVisitor)					AS `Rsl`
	, fnStats_Get_Odds(
		  fnStats_Get_FCHandicap(PA.Pb_Home, PA.Pb_Draw, PA.Pb_Visit, 0.11)
...
WHERE
		PA.WeekNo >= 4;
--	AND LENGTH(FT.Last5_V) - LENGTH(REPLACE(REPLACE(REPLACE(FT.Last5_V,'L','X'),'P','X'),'X','')) >= 2
--    AND PA.OH > 1.95
--    AND FT.Odd_1 >= 1.4
--    AND PA.OH - PA.OV BETWEEN 0.75 AND 2.15;
    

--    AND FT.Last5_V Like 'L%'
--    AND FT.Last5_H  Like'P%';
--    AND IFNULL(OH.Scored,0) > IFNULL(OV.Scored,0)
--    AND PA.OH - PA.OV > 0.99
--    AND IFNULL(PF.OH,0) - IFNULL(PF.OV,0) > 0.99;
--    AND PA.OH - PA.OV	> 1.85;
--    AND (FT.Last5_H LIKE 'P%' OR FT.Last5_H LIKE 'L%' OR FT.Last5_V LIKE 'P%' OR FT.Last5_V LIKE 'L%');