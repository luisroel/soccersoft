DELIMITER $$
CREATE DEFINER=`soccer`@`localhost` PROCEDURE `spStats_Get_TableDataList`(
      IN From_p 			DATETIME
    , IN To_p				DATETIME
...
BEGIN
    ...
    IF AssociationId_p = 0 THEN
		IF TournamentId_p = 0 THEN
			...
			FROM (
				SELECT
					...
					, FT.Odd_X													AS `OX`
					, FT.Odd_2													AS `O2`
					, (1.0/FT.Odd_1 + 1.0/FT.Odd_X + 1.0/FT.Odd_X)				AS `VAL`
				...
				WHERE
					FT.Time BETWEEN From_p AND To_p
                    AND FT.FixtureStatusId IN (1, 2, 4)
				) DD
			ORDER BY
				  DD.DFV
				, DD.DPT DESC
		...
		ELSE
			...
			WHERE
				DD.TournamentId = TournamentId_p
			ORDER BY
				  DD.DFV
				, DD.DPT DESC
				, DD.DFT DESC
	...
	ELSEIF TournamentId_p = 0 THEN
		...
		WHERE
			DD.AssociationId = AssociationId_p
		ORDER BY
			  DD.DFV
			, DD.DPT DESC
			, DD.DFT DESC
    ...
    ELSE
		...
		WHERE
				DD.AssociationId 	= AssociationId_p
			AND DD.TournamentId 	= TournamentId_p
		ORDER BY
              DD.DFV
			, DD.DPT DESC
			, DD.DFT DESC
