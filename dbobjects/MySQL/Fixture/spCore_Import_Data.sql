...
--					SELECT SS.SeasonId INTO @SeasonId FROM tbCore_Season SS WHERE SS.TournamentId = @TournamentId AND TRIM(SS.Name) = Season_p;            
				...
				IF EXISTS(SELECT * FROM tbCore_SeasonPhase SP WHERE TRIM(SP.Name) = Tournament_p AND SP.SeasonId IN (
						SELECT SS.SeasonId FROM tbCore_Season SS INNER JOIN tbCore_Tournament TT ON	TT.TournamentId = SS.TournamentId
						AND TT.AssociationId = @AssociationId WHERE TRIM(SS.Name) = Season_p)) THEN
					SELECT SP.SeasonId INTO @SeasonId FROM tbCore_SeasonPhase SP WHERE TRIM(SP.Name) = Tournament_p AND SP.SeasonId IN (
							SELECT SS.SeasonId FROM tbCore_Season SS INNER JOIN tbCore_Tournament TT ON	TT.TournamentId = SS.TournamentId
							AND	TT.AssociationId = @AssociationId WHERE TRIM(SS.Name) = Season_p) LIMIT 1;
					...
					IF @HomeTeamId <> -1 AND @VisitorTeamId <> -1 THEN
						SET @CountRecords = (SELECT COUNT(*) FROM tbCore_Fixture FT 
													WHERE
