    ...
    while (@from_date < To_p) do
		...
		from (
			select
				...
				, (`fx`.`odd_x` - `fx`.`odd_x`)		as `dox`
				, (`fx`.`odd_2` - `fx`.`odd_2`)		as `do2`
			...
			from
				...
				left join (
					select
						  `dd`.`mask`
						, sum(case when `dd`.`rsl` = '1' then 1.0 else 0.0 end) / count(`dd`.`rsl`) as `prc_1`
						, sum(case when `dd`.`rsl` = 'X' then 1.0 else 0.0 end) / count(`dd`.`rsl`) as `prc_x`
						, sum(case when `dd`.`rsl` = '2' then 1.0 else 0.0 end) / count(`dd`.`rsl`) as `prc_2`
					from (
						select
